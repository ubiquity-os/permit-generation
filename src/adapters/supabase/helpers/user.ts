...
export class User extends Super {
  ...
  async getUserById(userId: number) {
    ...
    if (error) {
      throw logger.error(FAILED_TO_GET_USER, { userId, er: error });
    }
  ...
  async getUserIdByWallet(wallet: string) {
    ...
    if (error) {
      throw logger.error(FAILED_TO_GET_USER, { wallet, er: error });
    }
  ...
  async upsertUser(userId: number, username: string) {
    ...
    if (error) {
      throw logger.error("Failed to upsert user", { userId, username, er: error });
    }
  ...
  async deleteUser(userId: number) {
    ...
    if (error) {
      throw logger.error("Failed to delete user", { userId, er: error });
    }
